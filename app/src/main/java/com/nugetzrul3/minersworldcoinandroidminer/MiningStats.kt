...
class MiningStats : AppCompatActivity() {
    ...
    fun getmininginfoNOMP() {
        Timer().scheduleAtFixedRate(object:TimerTask() {
            override fun run() {
                Thread (Runnable {
                    var info1 = get("https://bmine.net/api/worker_stats?"+getIntent().getStringExtra("walletaddress")).jsonObject.getString("miner")
                    var info2 = get("https://bmine.net/api/worker_stats?"+getIntent().getStringExtra("walletaddress")).jsonObject.getString("immature")
                    var info3 = get("https://bmine.net/api/worker_stats?"+getIntent().getStringExtra("walletaddress")).jsonObject.getString("paid")
                    var info4 = get("https://bmine.net/api/worker_stats?"+getIntent().getStringExtra("walletaddress")).jsonObject.getString("totalHash")
                    var info5 = get("https://bmine.net/api/worker_stats?"+getIntent().getStringExtra("walletaddress")).jsonObject.getString("totalShares")
                    this@MiningStats.runOnUiThread(java.lang.Runnable {
                        ...
                        balance.setText("Your Immature Balance: " + info2.toString() + "MWC")
                        paid.setText("Paid out: " + info3.toString() + "MWC")
